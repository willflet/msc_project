...
with open('../data/'+metric+'_matrix_'+data+'.pickle','rb') as f:
...
plt.xlim((-960,-950))
plt.ylim((5015,5045))
plt.savefig('../images/'+metric+'/'+method+'/'+prefix+'endpoints.png',dpi=250)
plt.close()
...
plt.xlim((-960,-950))
plt.ylim((5015,5045))
plt.savefig('../images/'+metric+'/'+method+'/'+prefix+'beelines.png',dpi=250)
plt.close()

for k in range(K):
    if populations[k] < 100: continue
    plt.figure(figsize=(4,4))
    i=0
    for _,traj in trajs.iteritems():
        label = labels[i]
    ...
    for _,traj in trajs.iteritems():
        label = labels[i]
        if label==k:
            plt.plot(traj[:,0],traj[:,1],linewidth=0.5,color=pal[k%12],alpha=0.25/int(data[:-1]))
        i += 1
    plt.axes().set_aspect('equal','datalim')
    plt.axis('off')
    plt.xlim((-960,-950))
    plt.ylim((5015,5045))
    plt.savefig('../images/'+metric+'/'+method+'/'+prefix+'endpoints'+str(k)+'.png',dpi=250)
    plt.close()
    ...
    for _,OD in ends.iteritems():
        label = labels[i]
        if label==k:
            plt.plot(OD[::2],OD[1::2],linewidth=0.5,color=pal[k%12],alpha=0.5/int(data[:-1]))
        i += 1
    plt.axes().set_aspect('equal','datalim')
    plt.axis('off')
    plt.xlim((-960,-950))
    plt.ylim((5015,5045))
    plt.savefig('../images/'+metric+'/'+method+'/'+prefix+'beelines'+str(k)+'.png',dpi=250)
    plt.clf()
    plt.close()

with open('../data/populations/'+metric+method+prefix+'.pickle','wb') as f:
